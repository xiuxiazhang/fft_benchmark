...
using namespace std;
void fft1d(float *signal, long int row, Complex *r_signal, int batch){
...
//	x=(struct _MKL_Complex16*)malloc(sizeof(struct _MKL_Complex16)*row);
	...
	status = DftiCreateDescriptor( &my_desc_handle, DFTI_SINGLE, DFTI_COMPLEX, 1, l[0]);
...
void fft2d(float *signal, int row, int col, Complex *r_signal, int batch){
//	struct _MKL_Complex16 *x;
	DFTI_DESCRIPTOR_HANDLE my_desc_handle;
	MKL_LONG status, l[2];
//	x=(struct _MKL_Complex16*)malloc(sizeof(struct _MKL_Complex16)*row);
	...
	l[0] = row; l[1] = col;
	status = DftiCreateDescriptor( &my_desc_handle, DFTI_SINGLE, DFTI_COMPLEX, 2, l);
	status = DftiCommitDescriptor( my_desc_handle);
//warm up
	//status = DftiComputeForward( my_desc_handle, (struct _MKL_Complex16*)r_signal);
	...
	float gflops=batch*COUNT*row*col*5*log(row*col)/log(2)/(stop-start)*1.e-9;
	cout<<"row="<<row<<" col="<<col<<", "<<(stop-start)/COUNT<<" s/fft2d "<<gflops<<" gflops"<<endl;
	status = DftiFreeDescriptor(&my_desc_handle);
}

void fft3d(float *signal, int row, int col, int nz, Complex *r_signal, int batch){
//	struct _MKL_Complex16 *x; 
	DFTI_DESCRIPTOR_HANDLE my_desc_handle;
	MKL_LONG status, l[3];
//	x=(struct _MKL_Complex16*)malloc(sizeof(struct _MKL_Complex16)*row);
...
//warm up
	...
	float gflops=batch*COUNT*row*col*nz*5*log(row*col*nz)/log(2)/(stop-start)*1.e-9;
